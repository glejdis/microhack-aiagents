#!/usr/bin/env python
"""Challenge 0 (coach/setup) — upload the CLM corpus PDFs into SharePoint.

`scripts/seed_corpus.py` wires a SharePoint document library into Azure AI
...
population: it uploads every PDF under `challenge-0/data/` into your SharePoint
...
Run it once, before participants reach Challenge 0 / Task 6:

    python scripts/upload_corpus_to_sharepoint.py            # upload
    python scripts/upload_corpus_to_sharepoint.py --dry-run  # list, no calls
...
# Corpus subfolders that hold the grounding PDFs (mirrors challenge-0/data/).
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Corpus root to upload from (default: challenge-0/data).",
    )
    parser.add_argument(
        "--library",
    ...
    if not files:
        _fail(f"No PDFs found under {data_dir}. Run scripts/make_corpus_pdfs.py first.")
    ...
    print(
        ...
        f"Next: run 'python scripts/seed_corpus.py' to index the library into clm-corpus."
    )
