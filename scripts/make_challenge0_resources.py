#!/usr/bin/env python3
"""Render the Challenge 0 Azure-resources diagram.

Draws every Azure resource, the LLM model fleet, and the surrounding
identity / delivery plane that Challenge 0 provisions (via ``azd up`` or
``scripts/deploy.sh``) for the Contract Lifecycle Management microhack.

Outputs:
    challenge-0/images/challenge-0-azure-resources.png
    challenge-0/images/challenge-0-azure-resources.svg

Usage:
    python scripts/make_challenge0_resources.py
"""
...
text(80, 106.4,
     "Provisioned by  azd up  or  scripts/deploy.sh  into a single resource group "
...
# Save
# --------------------------------------------------------------------------
out_dir = pathlib.Path(__file__).resolve().parents[1] / "challenge-0" / "images"
