...
|--------|----------|---------|
| `contract_templates/` | Approved **NDA / MSA / SOW** templates (PDF) | Intake & Drafting agent (Ch1) |
| `clause_library/` | `standard_clauses.pdf` — enterprise-standard positions (CL-01…CL-12) | Clause & Risk agent (Ch3) |
...
| `contracts/` | 5 **executed** contracts (PDF), one per row in `contracts_seed.json` | Clause & Risk (Ch3), status/renewal tools |
| `counterparty_drafts/` | `acme_msa_draft.pdf`, `globex_nda_redline.pdf` — **inbound** drafts full of red flags | Clause & Risk agent (Ch3) |
| `playbooks/` | `negotiation_playbook.pdf` — fallback positions + escalation | Intake & Drafting (Ch1), grounding |
| `evaluation/` | `evaluation_dataset.jsonl` + `adversarial_prompts.jsonl` | Evaluation (Ch2), safety (Ch5) |
...
python scripts/make_corpus_pdfs.py
```

This rewrites every PDF under `challenge-0/data/`. The executed contracts and inbound drafts
