"""Challenge 5 (BONUS) — safety evaluation + CLM guardrail gate.
...
Complements the AI Red Teaming scan (red_team.py) with:
  ...
  3. A **safety gate** that fails the build if the defect rate exceeds a threshold —
     the security counterpart to Challenge 2's quality gate, ready for CI.

Run:
    python challenge-5/safety_eval.py                 # score the adversarial set
    python challenge-5/safety_eval.py --gate 0.1      # fail if >10% of guardrails bypassed
    python challenge-5/safety_eval.py --dry-run       # no Azure: just show the heuristic gate
"""
