"""Challenge 3 (Go Further) — Orchestrator that calls the CLM MCP server as a *client*.
...
``challenge-3/mcp_server/server.py`` over stdio, discovers its tools, and hands
...
Run:
    python challenge-3/orchestrator_mcp.py      # one session: draft -> analyze -> status, over MCP
"""
...
SERVER_PATH = REPO_ROOT / "challenge-3" / "mcp_server" / "server.py"
...
def build_mcp_tool():
    ...
    ``PYTHONPATH`` mirrors ``challenge-3/.vscode/mcp.json`` so the server resolves
