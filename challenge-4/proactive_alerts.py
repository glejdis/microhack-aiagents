"""Challenge 4 — proactive Teams alerts via the Bot Framework.
...
This module is runnable in two ways:
    python challenge-4/proactive_alerts.py --text "🔴 Contract CT-4821 renews in 30 days…"
    python challenge-4/proactive_alerts.py --from-renewals --days 30   # generate + send
...
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "challenge-4" / "agents"))
...
def _conversation_reference():
    ...
    if not (app_id and service_url and conversation_id):
        raise RuntimeError(
            ...
            "Publish the bot (Ch4), send it one message, and save the conversation reference. "
            "See challenge-4/README.md."
        )
